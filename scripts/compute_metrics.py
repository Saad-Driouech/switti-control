...
def main():
    ...
    # Load subset CSV for captions and filenames
    ...
    # Quality metrics (CLIP, PickScore, ImageReward)
    ...
    if modality and control_path:
        ...
        ctrl_metrics = calculate_control_metrics(
            pil_images, ctrl_tensors, modality, device=args_cli.device
        )
    ...
    # Write per-run result.json
    result_json = run_dir / "result.json"
    with open(result_json, "w") as f:
        json.dump(result, f, indent=2)
    print(f"[result] {json.dumps(result, indent=2)}")

    # Append to shared results.jsonl
    results_jsonl = out_dir / "results.jsonl"
    with open(results_jsonl, "a") as f:
        f.write(json.dumps(result) + "\n")

    # Rewrite results.csv from full jsonl
    rows = []
